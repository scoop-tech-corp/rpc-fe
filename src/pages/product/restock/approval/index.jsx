...
const ProductRestockApproval = (props) => {
  const [productRestockApprovalData, setProductRestockApprovalData] = useState([]);
  ...
  const onActionRestock = async (isAcceptedAll = 0, isRejectedAll = 0, reasonRejectAll = '') => {
    await productRestockApproval({
      id: props.id,
      productRestocks: isAcceptedAll || isRejectedAll ? [] : productRestockApprovalData,
      isAcceptedAll,
      isRejectedAll,
      reasonRejectAll
    })
    ...
    <>
      ...
      >
        <Stack spacing={3}>
          ...
          <ReactTable columns={columns} data={productRestockApprovalData} />
        </Stack>
      </ModalC>
      <ConfirmationC
...
ProductRestockApproval.propTypes = {
  id: PropTypes.number,
  open: PropTypes.bool,
  onClose: PropTypes.func
};

export default ProductRestockApproval;
