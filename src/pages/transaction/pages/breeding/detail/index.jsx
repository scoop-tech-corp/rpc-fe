...
import { acceptTransactionBreeding, deleteTransactionBreeding, getTransactionBreedingDetail } from '../service';
...
const TransactionDetail = (props) => {
  ...
  const onConfirm = async (val, procedure) => {
    if (val) {
      await acceptTransactionBreeding({
  ...
  const onDelete = async (val) => {
    if (val) {
      await deleteTransactionBreeding([id])
        .then((resp) => {
          if (resp.status === 200) {
            setDialog({ accept: false, reject: false, delete: false });
  ...
  const fetchData = async () => {
    const resp = await getTransactionBreedingDetail({
      id,
      ...filterLog
    });
