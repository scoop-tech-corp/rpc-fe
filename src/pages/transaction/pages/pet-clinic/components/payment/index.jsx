...
import {
  calculatePetClinicOutpatient,
  createPaymentPetClinicOutpatient,
  createPaymentPetClinicInpatient,
  getActiveTodayPromos,
  getBeforePayment,
  getPrepaymentsPetClinic,
  getPaymentMethodsOutpatient,
...
const Payment = (props) => {
  const { data } = props;
  const isInpatient = data?.isInpatient === true;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch prepayments untuk inpatient
      if (isInpatient) {
        try {
          const ppResp = await getPrepaymentsPetClinic(data.transactionId);
          setTotalPrepaid(ppResp?.data?.totalPrepaid ?? 0);
        } catch (_e) {
          // fetch prepayment gagal — lanjut tanpa DP
        }
      }
  ...
  const buildItemsPayload = () => {
    ...
    return { recipes, services, products };
  };

  const runCalculate = async (promos) => {
    setPromoLoading(true);
    try {
      const { recipes, services, products } = buildItemsPayload();
      const resp = await calculatePetClinicOutpatient({
  ...
  const onSubmit = async () => {
    try {
      ...
      const paymentResp = isInpatient
        ? await createPaymentPetClinicInpatient(data.transactionId, formValue, totalPrepaid)
  ...
  const TABS = [
    intl.formatMessage({ id: 'info-and-prescription' }),
  ...
  const tab0InvalidMsg = !tab0Valid ? intl.formatMessage({ id: 'fill-unit-price-prescriptions' }) : '';
    ...
    <>
      ...
      >
        ...
        <TabPanel value={tabActive} index={0}>
          ...
          <SectionCard icon={<MedicalServicesIcon fontSize="small" color="primary" />} title={intl.formatMessage({ id: 'doctor-prescription' })}>
            {formValue.recipeList.length === 0 ? (
              <Alert severity="info" sx={{ borderRadius: 1 }}><FormattedMessage id="no-prescription-transaction" /></Alert>
            ) : (
              <ScrollX>
        ...
        <TabPanel value={tabActive} index={3}>
          ...
          {calculateDone && (
            ...
            >
              ...
              {/* ── Info DP rawat inap ── */}
              {isInpatient && totalPrepaid > 0 && (
                <Box sx={{ mt: 1, p: 1.5, bgcolor: 'success.lighter', borderRadius: 1, border: '1px solid', borderColor: 'success.light' }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography variant="body2" color="success.dark">
                      DP / Prepayment sudah dibayar
                    </Typography>
                    <Typography variant="subtitle2" fontWeight="bold" color="success.dark">
                      - Rp {totalPrepaid.toLocaleString('id-ID')}
                    </Typography>
                  </Stack>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mt={0.5}>
                    <Typography variant="caption" color="text.secondary">Sisa yang harus dibayar</Typography>
                    <Typography variant="subtitle1" fontWeight="bold" color="primary.main">
                      Rp {Math.max(0, formValue.summaryTotalPayment - totalPrepaid).toLocaleString('id-ID')}
                    </Typography>
                  </Stack>
                </Box>
              )}
          ...
          <SectionCard icon={<PaymentsIcon fontSize="small" color="primary" />} title={intl.formatMessage({ id: 'payment-method' })}>
            <Stack spacing={2}>
              {/* ── Sumber Pembayaran (Cash / Transfer / dll) ── */}
              <Stack spacing={0.5}>
