...
import PromoOfferTransactionPetSalon from './promo-offer';
...
const Payment = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // getInpatientTransactionPetSalon(data.transactionId)
      Promise.all([
  ...
  return (
    <>
      <ModalC
        title={<FormattedMessage id="payment-pet-salon" />}
        open={props.open}
        onOk={() => onSubmit()}
          ...
          <Grid item xs={12}>
            <Button
              ...
              onClick={async () => {
                ...
                const getRespPromo = await checkPromoTransactionPetSalon({
                  transactionPetSalonId: data.transactionId,
                  recipes,
                  services,
                  products
                });
