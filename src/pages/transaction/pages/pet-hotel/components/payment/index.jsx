...
import {
  ...
  getBeforePayment,
  printInvoicePetHotelOutpatient
} from '../../service';
...
import PromoOfferTransactionPetHotel from './promo-offer';
...
const Payment = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // getInpatientTransactionPetHotel(data.transactionId)
      Promise.all([
  ...
  const onSubmit = async () => {
    try {
      await createPaymentPetHotelOutpatient(data.transactionId, formValue);
      const resp = await printInvoicePetHotelOutpatient(data.transactionId, formValue);
      console.log('resp PRINT', resp);
      const message = `Transaction Payment Pet Hotel has been successfully`;
          ...
          <Grid item xs={12}>
            <Button
              ...
              onClick={async () => {
                ...
                const getRespPromo = await checkPromoTransactionPetHotel({
