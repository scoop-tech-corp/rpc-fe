...
import { updateMenuSetting, createMenuSetting } from '../service';

import ModalC from 'components/ModalC';
import PropTypes from 'prop-types';

const FormMenuSetting = (props) => {
  ...
  const onSubmit = async () => {
    const actionForm = () => {
      ...
      if (props.data?.id) {
        ...
        return updateMenuSetting(setParam);
      } else {
        return createMenuSetting(setParam);
      }
    };
    ...
    await actionForm()
      .then((resp) => {
        if (resp && resp.status === 200) {
          dispatch(snackbarSuccess(`Menu setting has been ${props.data?.id ? 'updated' : 'created'} successfully`));
  ...
  const renderTitle = () => (props.data?.id ? 'update-menu-setting' : 'create-menu-setting');
...
export default FormMenuSetting;
