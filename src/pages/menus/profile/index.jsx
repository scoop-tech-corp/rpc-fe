...
import { deleteMenuProfile, getMenuProfile } from './service';
...
import FormMenuProfile from './form';
...
const MenuProfile = () => {
  ...
  const onConfirm = async (value) => {
    if (value) {
      await deleteMenuProfile(selectedRow)
        .then((resp) => {
          if (resp.status === 200) {
            setDialog(false);
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: <FormattedMessage id="icon" />, accessor: 'icon' },
  ...
  return (
    <>
      <HeaderPageCustom title={<FormattedMessage id="menu-profile" />} isBreadcrumb={true} />
      ...
      {openForm.isOpen && (
        <FormMenuProfile
          ...
          data={openForm.data}
          onClose={() => setOpenForm({ isOpen: false, data: null })}
          setParams={setParams}
        />
      )}
    </>
