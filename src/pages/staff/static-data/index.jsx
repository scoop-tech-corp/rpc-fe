...
import { getStaffDataStatic, deleteStaffDataStatic } from './service';
...
const StaffDataStatic = () => {
  ...
  const onConfirm = async (value) => {
    if (value) {
      await deleteStaffDataStatic(selectedRow)
        .then((resp) => {
          if (resp.status === 200) {
            setDialog(false);
  ...
  const fetchData = async () => {
    const resp = await getStaffDataStatic(paramDataStaticList);
...
export default StaffDataStatic;
