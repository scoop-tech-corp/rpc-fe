  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        isNotSorting: true,
        Cell: ({ value }) => (
          <Chip label={STATUS_LABEL[value] || value} size="small" color={STATUS_COLOR[value] || 'default'} variant="light" />
        )
      },
      {
    ...
    [intl, onOpenEdit, onPrint]
  );

  // ── Render ──────────────────────────────────────────────────────────────────
  return (
    <>
      <HeaderPageCustom title={<FormattedMessage id="quotation" />} isBreadcrumb />
      ...
      <MainCard content={false}>
            ...
            <Stack direction={matchDownSM ? 'column' : 'row'} spacing={1.5} flexWrap="wrap" alignItems="flex-end">
              ...
              <Stack spacing={0.4}>
                ...
                >
                  <MenuItem value="">
                    <em>
                      <FormattedMessage id="all-status" />
                    </em>
                  </MenuItem>
              ...
              <Stack spacing={0.4}>
                ...
                >
                  <MenuItem value="">
                    <em>
                      <FormattedMessage id="all-service-type" />
                    </em>
                  </MenuItem>
              ...
              <Stack direction="row" spacing={1}>
                <Button size="small" variant="outlined" onClick={onResetFilter}>
        ...
        <Box sx={{ px: 2, py: 1.5, display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
          <Button variant="outlined" size="small" color="success" startIcon={<DownloadIcon />} onClick={onExportExcel}>
            Export Excel
          </Button>
          <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={onOpenCreate}>
      ...
      <Dialog open={statusDialog.open} maxWidth="xs" fullWidth>
            ...
            {statusDialog.status === 'sent' && (
              <>
                ...
                <Alert severity="info" sx={{ mt: 1.5, borderRadius: 1 }}>
                  PDF quotation akan dikirim otomatis ke <strong>email customer</strong> yang terdaftar. Status quotation akan berubah
                  menjadi <strong>Sent</strong>.
                </Alert>
              </>
            )}
        ...
        <DialogActions>
          <Button onClick={() => setStatusDialog({ open: false, status: '', row: null })}>
            <FormattedMessage id="cancel" />
          </Button>
          <Button variant="contained" onClick={onConfirmStatusChange} autoFocus>
      ...
      <Dialog open={convertDialog.open} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setConvertDialog({ open: false, row: null })}>
            <FormattedMessage id="cancel" />
          </Button>
      ...
      <Dialog open={confirmDialog.open} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setConfirmDialog({ open: false, type: '', row: null })}>
            <FormattedMessage id="cancel" />
          </Button>
          <Button variant="contained" color={confirmDialog.type === 'delete' ? 'error' : 'primary'} onClick={onConfirmAction} autoFocus>
