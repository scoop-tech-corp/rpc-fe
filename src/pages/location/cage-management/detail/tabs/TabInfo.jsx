    ...
    <Stack spacing={3}>
      ...
      {cage.isOccupied && (
        <Alert severity="warning">
          ...
          {cage.occupantCustomer && <> — pemilik: <strong>{cage.occupantCustomer}</strong></>}
      ...
      {!editing ? (
        <>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            <Typography variant="subtitle1" fontWeight={600}>Informasi Kandang</Typography>
            {isAdminOrManager && (
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
            {[
              ...
              ['Kondisi',
                <Chip key="cond" size="small"
                  label={condLabel[cage.conditionStatus]}
                  color={condColor[cage.conditionStatus]} />],
              ['Catatan', cage.notes || '-'],
            ].map(([label, val]) => (
              <Box key={label}>
                <Typography variant="caption" color="text.secondary">{label}</Typography>
                <Typography variant="body2" sx={{ mt: 0.3 }}>{val}</Typography>
              </Box>
            ))}
          </Box>
        </>
        ...
        <Stack spacing={2.5}>
          <Typography variant="subtitle1" fontWeight={600}>Edit Informasi Kandang</Typography>
          <Divider />

          <TextField label="Nama Kandang *" size="small" value={form.cageName}
            onChange={(e) => f('cageName', e.target.value)} inputProps={{ maxLength: 100 }} />
          ...
          <Stack direction="row" spacing={2}>
            <TextField label="Kapasitas *" size="small" type="number" fullWidth
              inputProps={{ min: 1 }} value={form.capacity} onChange={(e) => f('capacity', e.target.value)} />
            <TextField label="Jumlah Unit *" size="small" type="number" fullWidth
              inputProps={{ min: 1 }} value={form.amount} onChange={(e) => f('amount', e.target.value)} />
          </Stack>

          <TextField label="Catatan" size="small" multiline rows={2}
            inputProps={{ maxLength: 300 }} value={form.notes} onChange={(e) => f('notes', e.target.value)} />

          <Stack direction="row" spacing={1}>
            <Button variant="outlined" onClick={() => setEditing(false)}>Batal</Button>
